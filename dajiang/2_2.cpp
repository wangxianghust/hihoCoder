...
void parse_time(vector<string> &input, vector<set<int32_t> > &times){
    int size = input.size();
    for(int i=0; i<size; ++i){
        string time = input[i];
        ...
        int index = time_from_start/ONE_DAY;
        if(times[index].size() < 2){
            times[index].insert(time_from_start);
        } else {
            if(time_from_start < *times[index].begin() || time_from_start > *(times[index].end()--)){
                times[index].insert(time_from_start);
            }
        }
    }
...
int main(){
    ...
    vector<set<int32_t> > times(31, set<int32_t>());
    ...
    for(int i=0; i<size; ++i){
        int ret = 0;
        int start = ( *(times[i].begin() ) - OFFSET) % ONE_DAY;
        int end = ( *(times[i].end()--)  - OFFSET) % ONE_DAY;
        if(start >= NOON_END || end < NOON_START){
            ret = end - start + 1;
        } else {
            int left = (NOON_START > start) ? (NOON_START - start) : 0;
        ...
        //int ret = *(times[i].end()--) - *(times[i].begin() );
        if(ret > 0){
            cout << "08." + int2str(i) + " " + to_string(ret) << endl;
        }
    }
